...
"""
class MyLibriSpeech(LIBRISPEECH):
    max_audio_length = 0
    ...
    # returns waveform and transcript
    def __getitem__(self, n: int) -> Tuple[Tensor, int, str, int, int, int]:
        metadata = self.get_metadata(n)
        waveform = _load_waveform(self._archive, metadata[0], metadata[1])
        if self.padding:
            waveform = torch.nn.functional.pad(waveform, (0, self.max_audio_length - waveform.size(1)))
            transcript = tuple([metadata[2].ljust(self.max_transcript_length, " ")])
        else:
            transcript = tuple([metadata[2]])
        # print("[getItem] transcript", transcript)
        return (waveform,) + transcript
"""

_DATA_SUBSETS = ["dev-clean", "dev-other", "test-clean", "test-other", "train-clean-100", "train-clean-360",
                 "train-other-500",]
_CHECKSUMS = {
...
def _get_librispeech_metadata(fileid: str, root: str, folder: str, ext_audio: str, ext_txt: str) \
    ...
    return (filepath, 16000, transcript, int(speaker_id), int(chapter_id), int(utterance_id),)


# modified version of torchaudio LIBRISPEECH class
class MyLibriSpeech(Dataset):
    ...
    def __init__(self, root: Union[str, Path], url: str = "train-clean-100", folder_in_archive: str = "LibriSpeech",
                 download: bool = False, dataset_lines=[], char2index={}) -> None:
        self._url = url
        ...
        if not os.path.isdir(self._path):
            if download:
                ...
                raise RuntimeError(f"Dataset not found at {self._path}. Please set `download=True` to download the "
                                   f"dataset.")

        # self._walker = sorted(str(p.stem) for p in Path(self._path).glob("*/*/*" + self._ext_audio))
        self._walker = []
        for i, p in enumerate(sorted(Path(self._path).glob("*/*/*" + self._ext_audio)), start=1):
            if i in dataset_lines:
                self._walker.append(str(p.stem))
